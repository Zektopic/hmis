...
public class AntibioticController implements Serializable {
    ...
    public List<Antibiotic> completeAntibiotic(String query) {
        ...
        if (query == null) {
            ...
            sql = "select c from Antibiotic c where c.retired=false and (upper(c.name)) like :q order by c.name";
    ...
    public List<Antibiotic> getSelectedItems() {
        if (selectText.trim().equals("")) {
            ...
            String sql = "select c from Antibiotic c where c.retired=false and (upper(c.name)) like :q order by c.name";
            java.util.Map<String, Object> m = new java.util.HashMap<>();
