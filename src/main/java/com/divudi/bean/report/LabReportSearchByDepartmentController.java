...
public class LabReportSearchByDepartmentController implements Serializable {
    ...
    public void searchAll() {
        ...
        if (txtSearch != null) {
            ...
            m.put("txtSearch", "%" + txtSearch.toUpperCase() + "%");
            m.put("txtSearchPhone", "%" + txtSearch + "%");
            sql = "select pi from PatientInvestigation pi join pi.investigation i join pi.billItem.bill b join b.patient.person p where (upper(p.name) like :txtSearch or upper(b.insId) like :txtSearch or p.phone like :txtSearchPhone or upper(i.name) like :txtSearch ) order by pi.id desc";
    ...
    public List<PatientInvestigation> getPatientInvestigations() {
        ...
        if (patientInvestigations == null) {
            ...
            if (txtSearch == null || txtSearch.trim().equals("")) {
                ...
                m.put("txtSearch", "%" + txtSearch.toUpperCase() + "%");
                m.put("txtSearchPhone", "%" + txtSearch + "%");
                sql = "select pi from PatientInvestigation pi join pi.investigation i join pi.billItem.bill b join b.patient.person p where (upper(p.name) like :txtSearch or upper(b.insId) like :txtSearch or p.phone like :txtSearchPhone or upper(i.name) like :txtSearch ) and b.createdAt between :fromDate and :toDate order by pi.id desc";
