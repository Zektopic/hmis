...
//        }
    ...
    public List<Staff> completeStaff(String query) {
        ...
        if (query == null) {
            ...
            if (getSpeciality() != null) {
                m.put("s", getSpeciality().getId());
                sql = "select p from Staff p where p.retired=false and (upper(p.person.name) like :q or upper(p.code) like :q) and p.speciality.id = :s order by p.person.name";
            } else {
