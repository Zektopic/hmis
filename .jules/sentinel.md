...
**Learning:** Raw string concatenations for `LIKE` clauses in JPQL (e.g., `(c.name) like '%" + val + "%'`) are highly vulnerable to injection. Also, if applying  to user input, the JPQL column should be explicitly transformed via `upper()` (e.g., `upper(c.name) like :param`) to correctly mimic the case-insensitive search logic.
